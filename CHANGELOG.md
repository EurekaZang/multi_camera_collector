...
### 数据集合并系统
- **多数据集合并**: 新增`merge_datasets`工具，可合并多个独立采集的数据集
- **智能冲突解决**: 支持三种时间戳冲突解决策略（offset/skip/overwrite）
- **统一CSV生成**: 自动生成与`make_dataset.py`完全相同格式的CSV索引文件
- **元数据保留**: 完整保留所有camera_info和统计信息
- **合并信息记录**: 自动生成merge_info.json记录合并详情

### 命令行工具
- **Python命令**: `merge_datasets --output merged dataset1/ dataset2/`
- **Shell脚本**: `./merge_datasets.sh --output merged dataset1/ dataset2/`
- **Launch文件**: 支持通过ROS2 launch启动合并任务
- **灵活配置**: 支持自定义输出格式和冲突策略

### 文档和测试
- **详细文档**: 新增MERGE_GUIDE.md完整使用指南
- **测试脚本**: 包含单元测试验证合并功能
- **使用示例**: 提供多种使用场景的示例代码
